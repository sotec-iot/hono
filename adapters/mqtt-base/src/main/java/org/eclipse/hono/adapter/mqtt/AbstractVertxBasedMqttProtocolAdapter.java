...
public abstract class AbstractVertxBasedMqttProtocolAdapter<T extends MqttProtocolAdapterProperties>
    ...
    public class MqttDeviceEndpoint {
        ...
        private Future<Subscription> registerCommandSubscription(final CommandSubscription cmdSub, final Span span) {
            ...
            return createCommandConsumer(cmdSub, span)
                    .map(consumer -> {
                        ...
                        final Pair<CommandSubscription, ProtocolAdapterCommandConsumer> existingCmdSub = commandSubscriptions
                                .get(cmdSub.getKey());
        ...
        private Future<ProtocolAdapterCommandConsumer> createCommandConsumer(final CommandSubscription subscription, final Span span) {
        ...
        private void afterCommandPublished(
            ...
            if (requiresPuback) {
                final Handler<Integer> onAckHandler = msgId -> {
                    ...
                    final Command command = commandContext.getCommand();
                    if (command.isOneWay() && command.isValid() && command.isAckRequired() && commandContext instanceof AbstractCommandContext<?>) {
                        ((AbstractCommandContext<?>) commandContext).sendDeliverySuccessCommandResponseMessage(HttpURLConnection.HTTP_ACCEPTED,
        ...
        private Future<Void> closeCommandConsumer(
            ...
            return commandConsumer.close(sendDisconnectedEvent, span.context())
                    .recover(thr -> {
        ...
        protected final void close(final String reason, final boolean sendDisconnectedEvent) {
            ...
            onCloseInternal(span, reason, sendDisconnectedEvent, true)
                    .onComplete(ar -> span.finish());
        }
        ...
        protected final void onClose() {
            ...
            onCloseInternal(span, reason, true, closeCommandConsumers)
                    .onComplete(ar -> span.finish());
        }
